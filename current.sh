...
if [ "${arr[6]}" == "false" ]; then
    ...
    echo "Staking status is ${arr[6]}"  echo "$current_date ERROR staking status is false - validtime: ${arr[0]}, haveconnections: ${arr[1]}, walletunlocked: ${arr[2]}, mintablecoins: ${arr[3]}, enoughcoins: ${arr[4]}, mnsync: ${arr[5]}, staking status: ${arr[6]}" >> $log_path_file
